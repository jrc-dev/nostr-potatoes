...
const { getEventHash, verifySignature, validateEvent, relayInit } = require('nostr-tools');
...
async function removeById(e, act = "removing") {
    return new Promise(async (resolve) => {
        ...
        if (!event) {
            resolve({ ok: false, message: "Event review wasn't found for your Nostr Public Key" });
            return
        }
        let eventDelete = {
            kind: 5,
        ...
        if (ok && veryOk) {
            ...
            resolve({ ok: false, message: "Error removing event" });
        }
    });
}
...
function watchRateEvents(e) {
    ...
    subscribeRemoved.on('event', event => {
        console.log('got event removed:', event)
        debugger
        if (event) {
            veryOk = verifySignature(event);
            const isProbablyMyEvent = (Date.now() - event.created_at * 1000) <= 5000;
...
async function calculateAverage(e) {
    return new Promise(async (resolve) => {
        let myContactList = await getContactList(e);
        ...
        let myFriendsReviews = await Promise.all(myFriendsReviewPromises);
        ...
        if (totalFriendsReviews > 0)
            resolve(sumOfRates / totalFriendsReviews);
        else
            resolve(0);
    });
}
